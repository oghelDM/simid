...
export class SimidProtocol {
	private listeners_: any;
	private sessionId_: any;
	private nextMessageId_: any;
	private target_: any;
	private resolutionListeners_: any;
	private blockedStep: any;
	...
	constructor() {
		...
		// this.target_.document.addEventListener("keydown", (event) => {
		...
		this.blockedStep = queryParams.get("blockedStep");
	}
	 ...
	 */
	sendMessage(messageType: any, messageArgs?: any) {
	 ...
	 */
	addListener(messageType: any, callback: any) {
	 ...
	 * @private
	 */
	addResolveRejectListener_(messageId: any, resolve: any, reject: any) {
	...
	receiveMessage(event: any) {
		...
		if (Object.values(ProtocolMessage).includes(type)) {
			...
			if (this.blockedStep === specificType) {
				return;
			}
			if (listeners) {
	 ...
	 * @private
	 */
	handleProtocolMessage_(data: any) {
		const type = data["type"];
		switch (type) {
			...
			case ProtocolMessage.REJECT:
				...
				const resolutionFunction =
					this.resolutionListeners_[correlatingId];
	 ...
	 */
	resolve(incomingMessage: any, outgoingArgs?: any) {
		const messageId = this.nextMessageId_++;
	 ...
	 * @param {!Object} outgoingArgs Any arguments that are part of the resolution.
	 */
	reject(incomingMessage: any, outgoingArgs: any) {
		const messageId = this.nextMessageId_++;
...
export const CreativeMessage = {
	...
	REQUEST_VOLUME: "Creative:requestChangeVolume",
 ...
 */
export const EventsThatRequireResponse = [
